...
7. In the GUI window that appears, select the .cdb file from step 4, the `ProjectIgnis` directory (on Windows, this is `C:\ProjectIgnis` if you did not change the default installation location), and which player number you are using. You may also make any of the following optional adjustments:
  ...
  * Checking the box to force card arts to download will cause all card arts to be downloaded (or copied from your `ProjectIgnis` directory) during the scramble process. If it is left unchecked, the card arts will be automatically downloaded in the background while EDOPro is open.
    * If card arts for newer cards are not appearing in your scramble, they may have not been added to the image repository yet, and rerunning the Scrambler with this box checked will force them to download from an alternate source.
    * If you already have many card arts downloaded in `ProjectIgnis\pics`, checking this box may be faster than letting EDOPro download the images in the background, as the Scrambler will be able to copy them instead of downloading them.
...
13. The scripts for your cards are saved in `ProjectIgnis\repositories\ygo-scrambler\script`. If you and your opponent are using separate scrambles, make sure the player who is hosting the duel has copies of both player's scripts.
14. If you are the host, when your opponent sends you their .cdb file and script files, put them in the same locations as yours. Make sure the scripts are not put in a subfolder in the `scripts` folder, or EDOPro may not be able to find them.
